...
| Check                       | Result                                                                                   |
| --------------------------- | ---------------------------------------------------------------------------------------- |
| `postgres`                  | healthy; `pg_isready -U portfolio` → _accepting connections_                             |
| `minio`                     | healthy via `/minio/health/live`                                                         |
| `createbucket`              | **exit 0**; `portfolio-media` created                                                    |
| Bucket is private (SPEC §9) | `mc anonymous get` → _`private`_; unauthenticated `GET /portfolio-media/` → **HTTP 403** |
...
- **Added a `healthcheck` to `minio` and changed `createbucket` to
  ...
  container to _start_, never for it to be _ready_, so `mc` races MinIO and dies with
...
- Only _host_ ports were remapped; internal service DNS is untouched, so `createbucket`
